...
int main()
{
  ...
  while(1);
     ofstream output_file("results.csv"); // Open the output file
    ...
    {
        for (double j = 1; j <= 50; j += 0.5)
        {
            Battery battery(30000);
            SuperCapacitor sp(i, j);
           ...
           int pace_count=battery.paced_count;//z
           //i , j, pace_count
            output_file << i << "," << j << "," << pace_count << "\n";
